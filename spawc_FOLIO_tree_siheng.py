...
if __name__ == '__main__':
    ...
    # Load the FOLIO FOL dataset into folio variable and convert into a pandas dataframe df_f
    #folio = "folio-train.jsonl"
    #folio = read_jsonl_file(folio)
    # df_f = pd.DataFrame(data=folio)
    ...
    for index in range(len(df_f)):
        ...
        for j in range(len(arr)):
            ...
            if tree is not None:
                ...
                #unique_grounded_entities.update(variable_entities)
        unp.append([unique_predicates, unique_grounded_entities])
        len_unp.append([len(unique_predicates), len(unique_grounded_entities)])
        mult_unp.append(2 ** (len(unique_predicates) * len(unique_grounded_entities) * len(unique_grounded_entities)))
    ...
    for subarray in df_temp:
        ...
        for k in range(len(sub_expr)):
            ...
            if tree is not None:
                ...
                print(unique_grounded_entities)
                print(unique_variable_entities)
            #num_q = num_quantifiers(rule.rule_str())
    ...
    for ikj in range(len(term_orders)):
        ...
        item_count = {}
        print(selected_quant_qual)
        variables = list(set(sum(selected_elements, [])))
        selected_dimacs = []
        selected_clauses = []
        for im in range(len(selected_truth_t)):
            dimacs, clauses = truth_table_to_cnf_dimacs(selected_truth_t[im], selected_elements[im], variables)
            selected_dimacs.append(dimacs)
            selected_clauses.append(clauses)

        num_elem = len(list(sum(selected_clauses, [])))
        write_dimacs_to_file(selected_clauses, story_id, num_elem, len(variables))
        story_id += 1
